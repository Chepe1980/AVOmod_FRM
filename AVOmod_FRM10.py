...
if uploaded_file is not None:
    ...
    # Plot logs with highlighting for selected points
    line1, = ax[0].plot(ll.VSH, ll.DEPTH, '-g', label='Vsh')
    line2, = ax[0].plot(ll.SW, ll.DEPTH, '-b', label='Sw')
    ...
    class SelectFromCollection:
        def __init__(self, ax, collection, alpha_other=0.1):
            ...
            if len(self.fc) == 0:
                self.fc = np.ones((self.Npts, 4))
                self.fc[:,0:3] = mcolors.to_rgba('b')[:3]
            elif len(self.fc) == 1:
    ...
    pts_orig = ax2[0].scatter(ll.IP, ll.VPVS, 20, ll.LFC_B, marker='o', edgecolors='none', 
                             alpha=0.3, cmap=cmap_facies, vmin=0, vmax=4)
    selector_orig = SelectFromCollection(ax2[0], pts_orig)
    ax2[0].set_xlim(3000,16000); ax2[0].set_ylim(1.5,3)
    ax2[0].set_title('Original Data')
    
    # Brine FRM crossplot
    pts_brine = ax2[1].scatter(ll.IP_FRMB, ll.VPVS_FRMB, 20, ll.LFC_B, marker='o', 
                              edgecolors='none', alpha=0.3, cmap=cmap_facies, vmin=0, vmax=4)
    selector_brine = SelectFromCollection(ax2[1], pts_brine)
    ax2[1].set_xlim(3000,16000); ax2[1].set_ylim(1.5,3)
    ax2[1].set_title('FRM to Brine')
    
    # Oil FRM crossplot
    pts_oil = ax2[2].scatter(ll.IP_FRMO, ll.VPVS_FRMO, 20, ll.LFC_O, marker='o', 
                            edgecolors='none', alpha=0.3, cmap=cmap_facies, vmin=0, vmax=4)
    selector_oil = SelectFromCollection(ax2[2], pts_oil)
    ax2[2].set_xlim(3000,16000); ax2[2].set_ylim(1.5,3)
    ax2[2].set_title('FRM to Oil')
    
    # Gas FRM crossplot
    pts_gas = ax2[3].scatter(ll.IP_FRMG, ll.VPVS_FRMG, 20, ll.LFC_G, marker='o', 
                            edgecolors='none', alpha=0.3, cmap=cmap_facies, vmin=0, vmax=4)
    selector_gas = SelectFromCollection(ax2[3], pts_gas)
    ax2[3].set_xlim(3000,16000); ax2[3].set_ylim(1.5,3)
    ...
    # Finalize AVO plot
    ax_avo.set_title(f"AVO Reflection Coefficients\nDepth: {middle_depth:.1f}-{middle_depth+window_size:.1f}m")

...
def create_interactive_crossplot(logs):
    ...
    try:
        ...
        # Handle NaN values in LFC_B and ensure they're integers
        logs['LFC_B'] = logs['LFC_B'].fillna(0).astype(int)
        logs['LFC_Label'] = [lfc_labels[x] if x in range(len(lfc_labels)) else 'Undefined' 
                            for x in logs['LFC_B']]
        
        # Filter out NaN values in the plotting columns
        plot_data = logs[['IP', 'VPVS', 'LFC_Label', 'DEPTH']].dropna()
        
        # Check if we have data to plot
        if len(plot_data) == 0:
            st.warning("No valid data available for crossplot (all values may be NaN)")
            return None

...
def plot_cwt_analysis(all_gathers, t_samp, angles, cwt_scales, cwt_wavelet, wavelet_freq):
    ...
    try:
        ...
        for col_idx, case in enumerate(all_gathers.keys()):
            ...
            cwt_magnitudes = np.zeros((len(t_samp), len(freqs)))
            
            for i in range(len(t_samp)):
                trace = syn_gather[:, i]  # All angles at this time sample
                if len(trace) == 0:
                    continue
                ...
                if coefficients.size > 0:
                    # Sum across angles for each scale
            ...
            # Plot CWT magnitude (frequency vs time)
            extent = [freqs[0], freqs[-1], t_samp[-1], t_samp[0]]
            ...
            ax_cwt[1, col_idx].set_xlim(t_samp[0], t_samp[-1])
            ...
            if cwt_magnitudes.size > 0:
                ...
                ax_cwt[2, col_idx].plot(t_samp, dominant_freqs, 'r-')

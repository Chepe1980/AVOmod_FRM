...
def plot_rock_physics_templates():
...
if uploaded_file is not None:
    try:
        ...
        if show_templates:
            st.header("Rock Physics Templates")
            ...
            # Create figure
            fig_templates, ax_templates = plt.subplots(figsize=(10, 8))
            ...
            for fluid, color in [('brine', 'blue'), ('oil', 'green'), ('gas', 'red')]:
                ...
                # Plot the template
                ax_templates.plot(points[hull.vertices,0], points[hull.vertices,1], 
                                '--', color=color, alpha=0.7, label=f'{fluid.capitalize()} Sand')
                ax_templates.fill(points[hull.vertices,0], points[hull.vertices,1], 
                                color=color, alpha=0.1)
            
            # Add shale trend
            shale_ip = np.linspace(7000, 9000, 10)
            shale_vpvs = np.linspace(1.8, 2.4, 10)
            ax_templates.plot(shale_ip, shale_vpvs, '--', color='black', label='Shale Trend')
            
            # Format plot
            ax_templates.set_xlabel('Acoustic Impedance (m/s*g/cc)')
            ax_templates.set_ylabel('Vp/Vs')
            ax_templates.set_title('Ødegaard & Avseth Rock Physics Templates')
            ax_templates.legend()
            ax_templates.grid(True)
            ...
            if not plot_export_options:
                ...
                for plot_name in plot_export_options:
                    if plot_name == "Well Log Visualization":
                        ...
                        continue
                    elif plot_name == "Rock Physics Templates" and show_templates:
                        success1, error1 = export_plot(fig_templates, "Rock_Physics_Templates", "rock_physics_templates.png")
                        success2, error2 = export_plot(fig_combined, "Well_Data_Overlay", "well_data_overlay.png")

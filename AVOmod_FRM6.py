...
if uploaded_file is not None:
    ...
    # AVO Modeling for Middle Interface Only
    st.header("Synthetic AVO Gathers (Middle Interface)")
    ...
    # Create figure for synthetic gathers
    ...
    for idx, case in enumerate(cases):
        ...
        im = ax3[idx].imshow(syn_gather.T, aspect='auto', extent=extent,
        ...
        ax3[idx].text(0.05, 0.95, props_text, transform=ax3[idx].transAxes,
                     fontsize=9, verticalalignment='top', bbox=dict(facecolor='white', alpha=0.7))
        
        ax3[idx].set_title(f"{case} Case", fontweight='bold')
        ax3[idx].set_xlabel("Angle (degrees)")
        ax3[idx].set_ylabel("Time (s)")
        ax3[idx].set_ylim(0.25, 0.15)  # Zoom to middle interface
        
        plt.colorbar(im, ax=ax3[idx], label='Amplitude')

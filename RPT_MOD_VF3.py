...
def plot_frequency_analysis(all_gathers, t_samp, angles, wavelet_freq):
    """Plot frequency domain analysis (FFT) with time on y-axis and angles on x-axis"""
    ...
    for idx, case in enumerate(all_gathers.keys()):
        ...
        freq_spectra = np.zeros((len(t_samp), len(angles)))
        
        for i, angle in enumerate(angles):
            trace = syn_gather[i]
            spectrum = np.abs(np.fft.rfft(trace))
            # Interpolate to match time axis
            freq_spectra[:, i] = np.interp(
                t_samp, 
                np.linspace(t_samp[0], t_samp[-1], len(spectrum)), 
                spectrum
            )
        ...
        # Plot with time on y-axis and angles on x-axis
        extent = [angles[0], angles[-1], t_samp[-1], t_samp[0]]  # Note: time inverted for seismic convention
        im = ax_freq[idx].imshow(
            freq_spectra, 
            aspect='auto', 
            extent=extent,
            cmap='jet', 
        ...
        ax_freq[idx].set_ylim(t_samp[-1], t_samp[0])  # Inverted for seismic display
            ...
            if not plot_export_options:
                ...
                for plot_name in plot_export_options:
                    if plot_name == "Well Log Visualization" and model_choice not in ["Soft Sand RPT (rockphypy)", "Stiff Sand RPT (rockphypy)"]:
                        ...
                        fig_freq, _ = plt.subplots(1, 3, figsize=(18, 5))
                        plot_frequency_analysis(all_gathers, t_samp, angles, wavelet_freq)
                        ...
                        fig_cwt, _ = plt.subplots(3, len(all_gathers), figsize=(18, 12))
                        plot_cwt_analysis(all_gathers, t_samp, angles, cwt_scales, cwt_wavelet, wavelet_freq)

...
def create_interactive_crossplot(logs):
    ...
    logs['LFC_Label'] = [lfc_labels[int(x)] if not np.isnan(x) else 'Undefined' for x in logs['LFC_B']]
    ...
    color_map = factor_cmap('LFC_Label', palette=Category10[len(unique_labels)], factors=unique_labels)

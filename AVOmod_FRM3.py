...
if uploaded_file is not None:
    ...
    # Create figure for AVO results with adjusted wavelet plot size
    fig3, ax3 = plt.subplots(1, 2, figsize=(12, 6), gridspec_kw={'width_ratios': [1, 2]})
    
    # Wavelet plot with color and smaller size
    ...
    for case in cases:
        ...
        # Shale properties (assuming constant below our zone)
    ...
    # Create synthetic gathers for each case with property annotations
    st.header("Synthetic Seismic Gathers")
    
    fig4, ax4 = plt.subplots(1, 3, figsize=(18, 6))
    
    for idx, case in enumerate(cases):
        # Get properties for lower zone
        vp_m = logs.loc[((logs.DEPTH >= ztopl) & (logs.DEPTH <= zbot)), case_data[case]['vp']].values.mean()
        vs_m = logs.loc[((logs.DEPTH >= ztopl) & (logs.DEPTH <= zbot)), case_data[case]['vs']].values.mean()
        rho_m = logs.loc[((logs.DEPTH >= ztopl) & (logs.DEPTH <= zbot)), case_data[case]['rho']].values.mean()

    ...
    try:
        ...
        for idx, case in enumerate(cases):
            ...
            fig_synth.add_trace(go.Heatmap(
                ...
                zmid=0,
                hoverongaps=False,
                customdata=np.dstack([np.full_like(syn_gather.T, angle) for angle in angles])[0],
                hovertemplate="Angle: %{customdata}°<br>Time: %{y:.3f}s<br>Amplitude: %{z:.2f}<extra></extra>",
        ...
        def heatmap_selection(trace, points, selector):
            if points.point_inds:
                st.session_state.selected_points = points.point_inds
                st.write(f"Selected {len(points.point_inds)} points in gather")
        
        for trace in fig_synth.data:
            if trace.type == 'heatmap':
                trace.on_click(heatmap_selection)

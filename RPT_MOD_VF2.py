...
and uncertainty analysis. Now with enhanced zoom functionality and improved visualizations.
""")
...
if uploaded_file is not None:
    try:
        ...
        st.header("Well Log Visualization with Zoom")
        ztop, zbot = st.slider(
            "Select Depth Range", 
        ...
        if model_choice not in ["Soft Sand RPT (rockphypy)", "Stiff Sand RPT (rockphypy)"]:
            # Create the well log figure with zoom controls
            fig, ax = plt.subplots(nrows=1, ncols=4, figsize=(16, 10))
            plt.subplots_adjust(bottom=0.2)  # Make room for zoom controls
            ...
            # Span selector for horizontal zoom
            span = SpanSelector(
                ax[0], onselect, 'horizontal', useblit=True,
                props=dict(alpha=0.5, facecolor='yellow')
            )

            # Cursor for better visibility
            cursor = Cursor(ax[0], useblit=True, color='red', linewidth=1)

            # Add zoom buttons
            ax_zoom = plt.axes([0.3, 0.05, 0.4, 0.075])
            btn_zoom_in = Button(ax_zoom, 'Zoom In')
            btn_zoom_out = Button(plt.axes([0.1, 0.05, 0.15, 0.075]), 'Zoom Out')
            btn_reset = Button(plt.axes([0.7, 0.05, 0.15, 0.075]), 'Reset View')

            def zoom_in(event):
                for i in ax[:-1]:
                    xlim = i.get_xlim()
                    center = np.mean(xlim)
                    width = (xlim[1] - xlim[0]) * 0.7  # Zoom factor
                    i.set_xlim(center - width/2, center + width/2)
                fig.canvas.draw_idle()
        ...
        # Enhanced 2D Crossplots
        st.header("Enhanced 2D Crossplots")
        fig2, ax2 = plt.subplots(nrows=2, ncols=2, figsize=(16, 12))
        
        # Original Data
        sc0 = ax2[0,0].scatter(logs.IP, logs.VPVS, 20, logs.LFC_B, marker='o', 
                              edgecolors='none', alpha=0.7, cmap=cmap_facies, vmin=0, vmax=4)
        ax2[0,0].set_title('Original Data')
        ax2[0,0].set_xlabel('IP (m/s*g/cc)')
        ax2[0,0].set_ylabel('Vp/Vs')
        ax2[0,0].grid(True)
        
        # Brine Case
        sc1 = ax2[0,1].scatter(logs.IP_FRMB, logs.VPVS_FRMB, 20, logs.LFC_B, marker='o', 
                              edgecolors='none', alpha=0.7, cmap=cmap_facies, vmin=0, vmax=4)
        ax2[0,1].set_title('FRM to Brine')
        ax2[0,1].set_xlabel('IP (m/s*g/cc)')
        ax2[0,1].set_ylabel('Vp/Vs')
        ax2[0,1].grid(True)
        
        # Oil Case
        sc2 = ax2[1,0].scatter(logs.IP_FRMO, logs.VPVS_FRMO, 20, logs.LFC_O, marker='o', 
                              edgecolors='none', alpha=0.7, cmap=cmap_facies, vmin=0, vmax=4)
        ax2[1,0].set_title('FRM to Oil')
        ax2[1,0].set_xlabel('IP (m/s*g/cc)')
        ax2[1,0].set_ylabel('Vp/Vs')
        ax2[1,0].grid(True)
        
        # Gas Case
        sc3 = ax2[1,1].scatter(logs.IP_FRMG, logs.VPVS_FRMG, 20, logs.LFC_G, marker='o', 
                              edgecolors='none', alpha=0.7, cmap=cmap_facies, vmin=0, vmax=4)
        ax2[1,1].set_title('FRM to Gas')
        ax2[1,1].set_xlabel('IP (m/s*g/cc)')
        ax2[1,1].set_ylabel('Vp/Vs')
        ax2[1,1].grid(True)
        ...
        cbar = fig2.colorbar(sc3, cax=cbar_ax)
        cbar.set_ticks([0.5, 1.5, 2.5, 3.5, 4.5])
        cbar.set_ticklabels(['Undef', 'Brine', 'Oil', 'Gas', 'Shale'])
        cbar.set_label('Litho-Fluid Class')
        ...
        if model_choice not in ["Soft Sand RPT (rockphypy)", "Stiff Sand RPT (rockphypy)"]:
            ...
            # Enhanced Synthetic Seismic Gathers
            st.header("Enhanced Synthetic Seismic Gathers")
            st.markdown("Use the sliders below to adjust the display range")

            # Add controls for gather display
            col1, col2 = st.columns(2)
            with col1:
                amp_scale = st.slider("Amplitude Scale", 0.1, 2.0, 1.0, 0.1)
            with col2:
                trace_spacing = st.slider("Trace Spacing", 0.5, 3.0, 1.5, 0.1)

            fig4, ax4 = plt.subplots(1, 3, figsize=(18, 8))
            ...
            for idx, case in enumerate(cases):
                ...
                # Plot with adjustable scaling
                extent = [angles[0]-trace_spacing/2, angles[-1]+trace_spacing/2, 
                          t_samp[-1], t_samp[0]]
                im = ax4[idx].imshow(syn_gather.T, aspect='auto', extent=extent,
                                    cmap=selected_cmap, 
                                    vmin=-np.max(np.abs(syn_gather))*amp_scale, 
                                    vmax=np.max(np.abs(syn_gather))*amp_scale)
                
                # Add wiggle traces
                for i, angle in enumerate(angles):
                    trace = syn_gather[i,:] * 0.5 * amp_scale * trace_spacing + angle
                    ax4[idx].plot(trace, t_samp, 'k', linewidth=0.5, alpha=0.7)
                ...
                ax4[idx].grid(True, alpha=0.3)
                
                # Add colorbar
                plt.colorbar(im, ax=ax4[idx], label='Amplitude')

            # Add scroll/zoom functionality
            ax_zoom = plt.axes([0.3, 0.05, 0.4, 0.05])
            zoom_slider = Slider(ax_zoom, 'Time Zoom', time_min, time_max, 
                                valinit=(time_min, time_max), valstep=0.01)

            def update_time(val):
                for a in ax4:
                    a.set_ylim(zoom_slider.val[1], zoom_slider.val[0])
                fig4.canvas.draw_idle()

            zoom_slider.on_changed(update_time)
